...
namespace SS
{
	...
	public:
		...
		constexpr int32 ITEM_SIZE() const { return sizeof(T); }
	...
	public:
		...
		void SetSizeDirectly(int32 newSize)
		{
			if(newSize > _allocator.GetCapacity())
			{
				Reserve(newSize);
		...
		void Reserve(int32 newCapacity)
		{
			...
			T* newData = _allocator.Alloc(__FILE__, __LINE__, newCapacity);
			if(prevData == newData)
			{
				return;
			}
		...
		FORCEINLINE int32 GetSize() const { return _size; }
		FORCEINLINE int32 GetCapacity() const { return _allocator.GetCapacity(); }
		FORCEINLINE T* GetData() const { return _allocator.GetData(); }

		FORCEINLINE const T& operator[](const int32 idx)  const
		{
		...
		FORCEINLINE T& operator[](const int32 idx)
		{
			if (IsValidIndex(idx) == false)
			{
				SS_INTERRUPT();
	...
	public:
		...
		private:
			iterator(int32 idx, PooledList& list) : _idx(idx), _list(list) { }
		};
		...
		private:
			const_iterator(int32 idx, const PooledList* list) : _idx(idx), _list(list) { }
		};
